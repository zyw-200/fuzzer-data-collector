...
def get_user_equafl_new(image_id, round_time):
...
def get_user_path_equafl_new(image_id, round_time):
	file_dir = "long_run/user/image_%d_2_%d" %(image_id, round_time)
	file = "%s/plot_data" %file_dir
	return path_extract(file)
...
def fuzz_data_process(image_id, round_time, count, httpd_list):
	...
	for round in range(1, 6):	
		...
		if firm_time_list[-1] < 24*3600:
			...
			last_crash  = firm_crash_list[-1]
			firm_crash_list.append(last_crash)
		print("Firmafl", image_id, round, time_to_first_crash(firm_time_list, firm_crash_list))
		...
		if full_time_list[-1] < 24*3600:
			...
			last_crash  = full_crash_list[-1]
			full_crash_list.append(last_crash)
		print("FULL", image_id, round, time_to_first_crash(full_time_list, full_crash_list))
...
def fuzz_data_process_path(image_id, round_time, count, httpd_list):
	...
	for round in range(1, 6):	
		...
		real_round = round
		time_list, crash_list = get_user_path_equafl(image_id, real_round-1)
		if time_list[-1] < 24:
			time_list.append(24)
			last_crash  = crash_list[-1]
			crash_list.append(last_crash)
		...
		# equafl+=
		time_list, crash_list = get_user_path_equafl_new(image_id, real_round-1)
		if time_list[-1] < 24:
			time_list.append(24)
			last_crash  = crash_list[-1]
			crash_list.append(last_crash)
